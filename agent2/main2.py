...
class Main2(object):
    def __init__(self, n, g):
        ...
        self.rl = DQN2(
            gra=g,
            s_dim=2 * self.n,
            a_dim=int(math.pow(int(self.env.road_range / self.env.beam_section),n)),
            batch_size=128,
            gamma=0.99,
            lr=0.01,
            epsilon=0.1,
        ...
        for episode in range(1000):
            ...
            while True:
                ...
                for k in range(self.n):
                    s = add_action1 // int(self.env.road_range / self.env.beam_section)  # 商
                    y = add_action1 % int(self.env.road_range / self.env.beam_section)  # 余数
                    b = b + [y]
                    add_action1 = s
                b.reverse()
                for i in b:
                ...
                for dim in range(self.n):
                    action.append(self.env.cars_posit[dim] - self.env.road_range / 2 + add[dim] * self.env.beam_section)
